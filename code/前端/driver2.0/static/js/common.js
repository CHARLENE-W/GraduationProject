...
function getDistanceBtwGP(geoA,geoB){
	var resA= decode_geohash(geoA);
	var lonA=(resA[0]+resA[1])/2;
	var latA=(resA[2]+resA[3])/2;
	var resB= decode_geohash(geoB);
	var lonB=(resB[0]+resB[1])/2;
	var latB=(resB[2]+resB[3])/2;
	return getDistanceBtwP(latA,lonA,latB,lonB);
}
...
//CSS annimation__________________Wql
function CSSmove(obj,attr,target,speed,callback){
	clearInterval(obj.timer);

	var current =parseInt(getLocation(obj,attr));

	if(current>target){
		speed=-speed;
	}
	obj.timer=setInterval(function(){
		var oldValue=parseInt(getLocation(obj,attr));
